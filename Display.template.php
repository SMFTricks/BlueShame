...
function template_main()
{
	...
	echo '
			<div class="pagesection">
				...
				<div class="pagelinks floatleft">', $txt['pages'], ': ', str_replace($corchete, "", $context['page_index']), !empty($modSettings['topbottomEnable']) ? $context['menu_separator'] . ' &nbsp;&nbsp;<a href="#lastPost"><strong>' . $txt['go_down'] . '</strong></a>' : '', '</div>
			</div>';
	...
	while ($message = $context['get_message']())
	{
		...
		echo '
				<div class="', $message['approved'] ? ($message['alternate'] == 0 ? 'windowbg' : 'windowbg2') : 'approvebg', '">
					<div class="date_post"><div>', $message['time'], '</div><span>', !empty($message['counter']) ? $txt['reply_noun'] . ' #' . $message['counter'] : $txt['read'] . ' '. $context['num_views'] . ' ' . $txt['times'] . '', '</span></div>
					<span class="topslice"><span></span></span>
		...
		if (!$message['member']['is_guest'])
		{
			...
			// Info title and... something?
			echo '
								<li class="extrainfo">
									<span class="title">', $txt['sh_info'], '</span>';
									
								// Show the member's gender icon?
								if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '' && !isset($context['disabled_fields']['gender']))
									echo '
											<span class="gender floatright">', $message['member']['gender']['image'], '</span>';	
											
								// Show online and offline buttons?
								if (!empty($modSettings['onlineEnable']) && !$message['member']['is_guest'])
									echo '
											<span class="user_status floatright">', $context['can_send_pm'] ? '<a href="' . $message['member']['online']['href'] . '" title="' . $message['member']['online']['label'] . '">' : '', '<img src="', $message['member']['online']['image_href'], '" alt="', $message['member']['online']['text'], '" />', $context['can_send_pm'] ? '</a>' : '', '</span>';
			...
			if (!isset($context['disabled_fields']['posts']))
				echo '
								<li class="postcount">', $txt['member_postcount'], ': <a href="'. $scripturl. '?action=profile;area=showposts;sa=messages;u='. $message['member']['id']. '">', $message['member']['posts'], '</a></li>';
	...
	echo '
			<div class="pagesection">
				...
				<div class="pagelinks floatleft">', $txt['pages'], ': ', str_replace($corchete, "", $context['page_index']), !empty($modSettings['topbottomEnable']) ? $context['menu_separator'] . ' &nbsp;&nbsp;<a href="#top"><strong>' . $txt['go_up'] . '</strong></a>' : '', '</div>
